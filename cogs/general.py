...
class General(commands.Cog):
    ...
    async def userinfo(self, ctx, *, user: discord.User=None):
        ...
        if member:
            ...
            status = "Unknown"
            statEmoji = "❓"
        embed = discord.Embed(description=f"{statEmoji}({status})\n" 
    ...
    async def emoji_add(self, ctx, name: Optional[str], emote_pic: Optional[str]):
        ...
        if not name:
            ...
            return
        if len(name) < 2:
            await ctx.send("The name of the emoji needs to be at least 2 characters long!")
            return
        try:
            added_emote = await ctx.guild.create_custom_emoji(name=name, image=emote_pic)
        except Forbidden:
        ...
        embed.set_footer(text=f"Added by {ctx.message.author.name}#{ctx.message.author.discriminator}")
        await ctx.send(embed=embed)

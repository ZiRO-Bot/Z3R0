import aiohttp
import cogs.api.anilistQuery as query
import difflib
import json
import re

from cogs.utilities.formatting import pformat
...
class AniList:
    ...
    async def fetch_id_with_name(self, name: str, _type_: str = None) -> dict:
        """Get id from name. **DEPECRATED**"""
        var = {"name": name}
        if _type_:
            var['atype'] = _type_.upper()
        q = await self.request(
            "query($name:String" + (
                ",$atype:MediaFormat" if _type_ else ""
            ) + "){Media(search:$name,type:ANIME" + (
                    ",format:$atype" if _type_ else ""
                ) + "){id}}",
            var,
        )
        if "data" in q:
